...
async def channel_settings(client, query):
    ...
    chat_title = await get_channel_title_fast(user_id, channel_id)
    caption_data, link_status = await asyncio.gather(
        get_channel_caption(channel_id),
        get_link_remover_status(channel_id)
    )
    ...
    if not caption_data or "caption" not in caption_data:
        ...
        caption_preview = "\n".join(x for x in [prefix, caption, suffix] if x)
    text = (
...
async def set_caption_menu(client, query):
    ...
    caption_display = current_caption or "None set yet."
    buttons = [
...
async def set_words_menu(client, query):
    ...
    channel_id = int(query.matches[0].group(1))
    user_id = query.from_user.id
    chat_title, blocked_words = await asyncio.gather(
        get_channel_title_fast(user_id, channel_id),
        get_block_words(channel_id)
    )
    words_text = blocked_words or "None set yet."
    text = (
...
async def suffix_prefix_menu(client, query):
    ...
    channel_id = int(query.matches[0].group(1))
    user_id = query.from_user.id
    chat_title, (suffix, prefix) = await asyncio.gather(
        get_channel_title_fast(user_id, channel_id),
        get_suffix_prefix(channel_id)
    )
    buttons = [
